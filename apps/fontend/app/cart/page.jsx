    ...
    async function handleSubmit(e){
        ...
        try{
            ...
            if(res.ok){
                ...
                clearCart()
                setUserInfo({country:"",city:"",email:"",address:"",postalCode:"",name:""})
            }
            else{
                                        ...
                                        (product)=> (
                                            <tr key={product._id}>
                                                <td className="cart-product">
                                                    <Image src={product.image} width={80} height={80} className="cart-product-image" alt={product.title}/>
                                                    {product.title}
                                                    <input type="number" value={product.quntity} min="1" onChange={(event)=> updateQuantity(product._id , Number(event.target.value))}/>
                ...
                cart.length > 0 && (
                    <div className="cart-box">
                        ...
                        <form className="cart-form" onSubmit={handleSubmit}>
                            ...
                            <input value={userInfo.address} onChange={handleChange} name="address" type="text" className="cart-input" placeholder="ادرس" />
                            <input value={userInfo.postalCode} onChange={handleChange} name="postalCode" type="text" className="cart-input" placeholder="کد پستی" />
                            <button type="submit" className="cart-button">پرداخت انلاین</button>
                        </form>
                    </div>
                )
            }
